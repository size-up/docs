...
:::info
May need to re-run the command to remove all workflows, because of the rate limit of the GitHub API.
:::

:::tip

- Remove the `--silent` flag to see the output of the command.
- Can run the command in the background by adding `&` to the end of the command.
